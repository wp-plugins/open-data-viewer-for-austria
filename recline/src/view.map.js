  ...
  _getGeometryFromRecord: function(doc){
    if (this.state.get('geomField')){
      ...
      if (typeof(value) === 'string') {
        if (value.indexOf("MULTILINESTRING ") !== -1) {
          value = value.replace("MULTILINESTRING ", "").substring(1,value.length);
          
          var x = value.split('), (');
          for (var i in x) {
            x[i] = x[i].replace(/\(/g, '').replace(/\)/g, '');
            x[i] = x[i].split(", ");
            for (var y in x[i]) {
              x[i][y] = x[i][y].replace(/\(/g, '').replace(/\)/g, '');
              x[i][y] = x[i][y].split(" ");
            }
          }
          return {
        ...
        if (value.indexOf("POINT ") == -1) {
          ...
          // When field looks like "POINT (LAT LON)"
          value = value.replace('POINT ' , "").replace('(', '').replace(')', '');
          var parts = value.split(' ');
